...
namespace DBMS.Functions
{
    static class CConvert
    {
        ...
        public static string Encrypt(string clearText)
        {
            try
            {
                //Функция шифрации текста
                string EncryptionKey = "DBMStudio";
                byte[] clearBytes = Encoding.Unicode.GetBytes(clearText);
                using (Aes encryptor = Aes.Create())
                {
                    var m = new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 };
                    Rfc2898DeriveBytes pdb = new Rfc2898DeriveBytes(EncryptionKey,m, 100000,HashAlgorithmName.SHA1);
